...
Typical workflow:
  python main.py collect --topic nofaults --duration 600
  python main.py collect --topic single_fault --duration 600
...
def build_parser() -> argparse.ArgumentParser:
    ...
    combo_parser = subparsers.add_parser(
        "refresh", help="Collect new data, retrain, then monitor a stream in one go"
    )
    combo_parser.add_argument(
        "--collect-topic",
        default="nofaults",
        choices=sorted(TOPICS.keys()),
        help="Stream to collect before training",
    )
    combo_parser.add_argument(
        "--collect-duration",
        type=int,
        default=300,
        help="Seconds to collect before retraining",
    )
    combo_parser.add_argument("--csv", type=Path, default=DEFAULT_CSV)
    combo_parser.add_argument("--model-path", type=Path, default=DEFAULT_MODEL)
    combo_parser.add_argument("--scaler-path", type=Path, default=DEFAULT_SCALER)
    combo_parser.add_argument("--window-size", type=int, default=DEFAULT_WINDOW_SIZE)
    combo_parser.add_argument("--epochs", type=int, default=40)
    combo_parser.add_argument("--batch-size", type=int, default=64)
    combo_parser.add_argument(
        "--monitor-topic",
        default="variable_setpoints",
        choices=sorted(TOPICS.keys()),
        help="Stream to monitor after retraining",
    )
    combo_parser.add_argument(
        "--threshold",
        type=float,
    ...
    combo_parser.add_argument(
        ...
        type=int,
        default=0,
        help="Seconds to monitor after retraining (0 = until interrupted)",
    )
...
def main(argv: List[str] | None = None) -> None:
    ...
    if args.command == "collect":
        ...
        logging.info(
            "Refreshing dataset: collect %ds of %s into %s",
            args.collect_duration,
            args.collect_topic,
            args.csv,
        )
        collect_stream(args.collect_topic, args.collect_duration, args.csv, args.broker)
        logging.info("Retraining model from %s", args.csv)
        train_lstm(
            csv_path=args.csv,
            model_path=args.model_path,
            scaler_path=args.scaler_path,
        ...
        logging.info(
            "Monitoring %s after refresh (threshold=%.2f, duration=%s)",
            args.monitor_topic,
            args.threshold,
            args.monitor_duration or "infinite",
        )
        monitor_stream(
            topic_key=args.monitor_topic,
            broker=args.broker,
            model_path=args.model_path,
            scaler_path=args.scaler_path,
            window_size=args.window_size,
            threshold=args.threshold,
            duration=args.monitor_duration or None,
        )
